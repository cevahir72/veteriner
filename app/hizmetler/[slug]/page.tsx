...
export default async function ServiceDetailPage(props: ServicePageProps) {
  ...
  return (
    <main className="min-h-screen bg-background px-4 pb-24 pt-32 text-on-surface sm:px-6 lg:px-8">
      <section className="mx-auto max-w-6xl border-b border-white/70 pb-12">
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-end">
          <div>
            ...
            <h1 className="mt-4 text-4xl font-semibold tracking-[-0.03em] text-primary sm:text-5xl lg:text-6xl">
              {article.title}
            </h1>
      ...
      <section className="mx-auto mt-12 grid max-w-6xl gap-12 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <div>
          <p className="text-lg leading-8 text-on-surface-variant">{article.intro}</p>

          <div className="mt-12 space-y-8">
            {article.sections.map((section) => (
              <section key={section.heading} className="rounded-3xl bg-surface-container-low p-6 sm:p-8">
        ...
        <aside className="h-fit rounded-3xl border border-outline-variant bg-surface-container-lowest p-6 shadow-sm lg:sticky lg:top-28">
          ...
          <Link
            className="mt-6 inline-flex rounded-lg bg-primary px-5 py-3 text-label-caps text-on-primary hover:bg-primary-container"
            href="/#contact"
          >
            İletişime Geç
          </Link>
